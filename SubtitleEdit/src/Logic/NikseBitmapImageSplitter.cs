namespace Nikse.SubtitleEdit.Logic
{
    ...
    public class NikseBitmapImageSplitter
    {
            ...
            for (int x = 0; x < bmp.Width - 1; x++)
            {
                    ...
                    !IsVerticalLineTransparent(bmp, ref tempY, x + 1))
                {
                    ...
                    if (cursiveOk)
                    {
                        ...
                        foreach (Point point in cursivePoints)
                        {
                            for (int fixY = point.Y; fixY < bmp.Height; fixY++)
                            {
                                b1.SetPixel(point.X - startX, fixY, Color.Transparent);
                            }
                        }
        ...
        private static IEnumerable<ImageSplitterItem> SplitHorizontalNew(ImageSplitterItem lineSplitterItem, int xOrMorePixelsMakesSpace)
        {
            ...
            for (int x = 0; x < bmp.Width; x++)
            {
                ...
                else if (width > 1)
                {
                    ...
                    {
                        bmp.MakeVerticalLinePartTransparent(begin, point.X, point.Y);
                    }
        ...
        {
            return points.Select(p => p.X).Concat(new[] { x }).Min();
        }

        private static int FindMaxX(List<Point> points, int x)
        {
            return points.Select(p => p.X).Concat(new[] { x }).Max();
        }
