namespace Nikse.SubtitleEdit.Logic.Forms
{
    ...
    {
        private List<string> interjectionList;
                ...
                if (indexOfColon > 0 && IsNotInsideBrackets(text, indexOfColon))
                {
                    ...
                    else
                    {
                            ...
                            line != line.ToUpper())
                        {
                            ...
                            newText.Length > 1 && 
                            indexOfColon > 15 && 
                            line.Substring(0, indexOfColon).Contains(' ') && 
                            Utilities.CountTagInText(line, ':') == 1 &&
                            ".?!".IndexOf(newText[newText.Length - 1]) < 0 && 
                            newText.LineEndsWithHtmlTag(true) &&
                            line != line.ToUpper())
                        {
                            if (pre.Contains("<i>") && line.Contains("</i>"))
                            {
            ...
            if (noOfNames > 0 && Utilities.GetNumberOfLines(newText) == 2)
            {
                ...
                if (arr.Length == 2 && arr[0].Length > 1 && arr[1].Length > 1)
                {
                    ...
                    if (arr0.Length > 0 && arr1.Length > 1 &&
                        !(tempArr0QuoteTrimmed.EndsWith('.') || 
                        tempArr0QuoteTrimmed.EndsWith('!') || 
                        tempArr0QuoteTrimmed.EndsWith('?') || 
                        tempArr0QuoteTrimmed.EndsWith("</i>", StringComparison.Ordinal)) &&
        ...
        private static bool IsNotInsideBrackets(string text, int colonIdx)
        {
            ...
            return nIdx <= colonIdx;
        }

        private static bool DoRemove(string pre)
        {
        ...
        public string RemoveTextFromHearImpaired(string text)
        {
            ...
            {
                if (Settings.RemoveIfTextContains.Any(removeIfTextContain => text.Contains(removeIfTextContain)))
                {
                    return string.Empty;
                }
            }
        ...
        private static string GetRemovedString(string oldText, string newText)
        {
            ...
            var result = start > 0 ? oldText.Substring(0, oldText.Length - newText.Length) : oldText.Substring(newText.Length);
            result = result.Trim(' ', '(', ')', '[', ']', '?', '{', '}');
            return result;
        }
        ...
        public string RemoveInterjections(string text)
        {
            ...
            {
                interjectionList = new List<string>();
                foreach (string s in arr)
                {
                    ...
                    if (!interjectionList.Contains(upper))
                    {
                        interjectionList.Add(upper);
                    }

                    string pascalCasing = char.ToUpper(s[0]) + s.Substring(1);
                    if (!interjectionList.Contains(pascalCasing))
                    {
                        interjectionList.Add(pascalCasing);
                    }
                }

                interjectionList.Sort(CompareLength);
            }
                ...
                foreach (string s in interjectionList)
                {
                    if (text.Contains(s))
                    {
                        ...
                        if (match.Success)
                        {
                            ...
                            {
                                switch (temp.Substring(index - s.Length + 1, 3))
                                {
                                    case ", !":
                            ...
                            {
                                switch (temp.Substring(index - s.Length, 3))
                                {
                                    case ", !":
                                    ...
                                    case ", .":
                                        ...
                                        break;
                                    default:
                                        if (index > 0 && temp.Substring(index - s.Length).StartsWith(", -—"))
                                        {
                            ...
                            if (removeAfter && temp.Length > index - s.Length + 2 && index > s.Length)
                            {
                                switch (temp.Substring(index - s.Length + 1, 2))
                                {
                                    case "-!":
        ...
        {
            return Settings.OnlyIfInSeparateLine ? StartAndEndsWithHearImpariedTags(text) : HasHearImpairedText(text);
        }
