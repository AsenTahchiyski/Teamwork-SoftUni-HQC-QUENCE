namespace Nikse.SubtitleEdit.Logic.Forms
{
    ...
    public static class FixCommonErrorsHelper
    {
        ...
        public static string FixDialogsOnOneLine(string text, string language)
        {
            ...
            switch (Utilities.GetNumberOfLines(text))
            {
                case 2:
                {
        ...
        {
            Paragraph paragraph = subtitle.Paragraphs[i];
                ...
                text.Contains(Environment.NewLine + "<I> -"))
            {
                ...
                {
                    var noTaglines = HtmlUtil.RemoveHtmlTags(paragraph.Text).SplitToLines();
            ...
            else if (text.StartsWith("<font ", StringComparison.Ordinal))
            {
                var prev = subtitle.GetParagraphOrDefault(i - 1);
                if (prev != null &&
                    HtmlUtil.RemoveHtmlTags(prev.Text).TrimEnd().EndsWith('-') &&
                    !HtmlUtil.RemoveHtmlTags(prev.Text).TrimEnd().EndsWith("--", StringComparison.Ordinal))
                {
                    return text;
                }
        ...
        public static string FixHyphensAdd(Subtitle subtitle, int i, string language)
        {
            ...
            var lines = HtmlUtil.RemoveHtmlTags(paragraph.Text).SplitToLines();
            ...
            if (startHyphenCount == 1 && totalSpaceHyphen == 0)
            {
                ...
                if (parts.Length == 2)
                {
                    ...
                    // PERFORMANCE: possible performance bottleneck
        ...
        internal static string FixShortLines(string text)
        {
            ...
            if (!s.Contains(Environment.NewLine) || 
                s.Replace(Environment.NewLine, " ").Replace("  ", " ").Length >= Configuration.Settings.Tools.MergeLinesShorterThan)
            {
                return text;
            }
